...
def display_cameras():
    cap1 = initializecamera(0)  # 첫 번째 카메라 (0번 장치)
    cap2 = initializecamera(2)  # 두 번째 카메라 (1번 장치)
    ...
    while True:
        ...
        # 모델을 통한 처리 
        frame1_resized = cv2.resize(frame1, (480, 480))
        frame1 = run_object_detection(frame1_resized)
        frame2 = crack_segmentation(frame2)

        # 두 프레임을 세로로 결합 (하나의 화면에 두 카메라 화면 출력)

...
#define CALL_DEV_NAME "keyled_dev"
...
static long call_ioctl (struct file *filp, unsigned int cmd, unsigned long arg)
{
		...
		switch( cmd )
		{
				...
				case TIMER_VALUE :
                    err = copy_from_user((void *)&key_ctrl_info,(void *)arg, size);
					if (err != 0) {
							return -EFAULT;
					}
					timerVal = key_ctrl_info.timer_val;
					mod_timer(&timerLed, timerVal);
					break;
...
static __poll_t call_poll(struct file * filp, struct poll_table_struct * wait)
{

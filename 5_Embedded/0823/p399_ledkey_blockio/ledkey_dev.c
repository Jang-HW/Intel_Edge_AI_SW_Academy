...
//
	if(pKeyData->keyNumber == 0)
	{
			...
			{
					// when there are interrupt, this will be work 
					wait_event_interruptible(WaitQueue_Read, pKeyData->keyNumber);
			}
	}
//*/

	// in polling way, if change the value, this way can make having interrupt
//	wait_event_interruptible_timeout(WaitQueue_Read, 1, 100);

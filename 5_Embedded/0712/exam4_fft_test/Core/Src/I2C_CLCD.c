...
void I2C_CLCD_CustomFontVertical() {
	I2C_CLCD_SendByte(0, 0x40);
	for(int i = 0; i < 8; i++)	// PutC(1)
		I2C_CLCD_SendByte(1, 0b00000);
	for(int i = 0; i < 8; i++)	// PutC(2)
...
// n : 0 ~ 16 / vertical line binding
void I2C_CLCD_ProgressbarVertical(uint8_t n, uint8_t column) {
	I2C_CLCD_GotoXY(column, 0);
	if (n > 7) I2C_CLCD_PutC(n - 8);
	else if (n == 16) I2C_CLCD_PutC(0xff);
	else I2C_CLCD_PutC(0x01);

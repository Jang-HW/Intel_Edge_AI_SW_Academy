...
char recvId[10] = "JHW_BT";  // SQL 저장 클라이이언트 ID
...
void loop()
{
  ...
  {
    timerIsrFlag = false;
    // 가변저항의 범위가 0~1023 사이라서 0~100 사이로 매칭 
...
void bluetoothEvent()
{
  ...
  if (!strcmp(pArray[1], "LED")) {
    if (!strcmp(pArray[2], "ON")) {
      ...
      // 하드코딩으로 바로 sql 클라이언트로 보냄 
      sprintf(sendBuf, "[JHW_SQL]SET@%s@1@%s\n", pArray[1], pArray[0]);
    }
    else if (!strcmp(pArray[2], "OFF")) {
      digitalWrite(LED_BUILTIN_PIN, LOW);
      // 하드코딩으로 바로 sql 클라이언트로 보냄 
      sprintf(sendBuf, "[JHW_SQL]SET@%s@0@%s\n", pArray[1], pArray[0]);
    }
  ...
  //  sprintf(sendBuf, "[%s]%s@%s\n", pArray[0], pArray[1], pArray[2]);
    // 하드코딩으로 바로 sql 클라이언트로 보냄 
    sprintf(sendBuf, "[JHW_SQL]SET@%s@%s@%s\n", pArray[1], pArray[2], pArray[0]);

    // 전송을 해야하므로 return을 주석처리
    // return;
  }

...
//  HAL_ADC_Start_DMA(&hadc1, &adcValue, 1);
  ...
  HAL_ADC_Start_DMA(&hadc1, (uint32_t *)adcValue, 128);  // Do not need anything to read sound reference
...
//  u8g2_SendBuffer(&u8g2);
  ...
  while (1)
  {
  	if(doConvert == 0){
    	...
    	u8g2_ClearBuffer(&u8g2);
    	for(int x = 0; x <128; x++){
    		u8g2_DrawPixel(&u8g2, x, adcValue[x]/64);
    	}
    	u8g2_SendBuffer(&u8g2);
  	}
    /* USER CODE END WHILE */

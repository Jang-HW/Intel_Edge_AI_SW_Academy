    ...
    , ui(new Ui::MainWidget)
{
    ...
    // tab2에서 수신 받은 센서 값을 tab3에 전달하기 위한 시그널
    connect(pTab2SocketClient, SIGNAL(tab3RecvDataSig(QString)), pTab3ControlPannel, SLOT(tab3RecvDataSlot(QString)));

    // tab7에서 보안모드의 경고 문구를 전달하기 위한 시그널
    connect(pTab7CamSecurity->pGetCameracapture(), SIGNAL(sigSocketSendData(QString)), pTab2SocketClient->getSocketClient(), SLOT(slotSocketSendData(QString)));
}
